...
def get_download_folder():
    # Use the pathlib library to determine the Downloads folder
...
if st.button("Download"):
    if url:
        ...
        try:
            ...
            st.write(f"Download folder: {download_folder}")  # Debugging step
            if not os.path.exists(download_folder):
                st.error(f"Download folder does not exist: {download_folder}")
            else:
